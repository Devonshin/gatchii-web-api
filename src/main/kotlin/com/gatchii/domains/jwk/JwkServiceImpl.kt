...
class JwkServiceImpl(
    ...
    override suspend fun convertAlgorithm(jwk: JwkModel): Algorithm{
        val privateKey = convertPrivateKey(jwk.privateKey)
        val publicKey = convertPublicKey(jwk.publicKey)
        val algorithm = Algorithm.ECDSA256(publicKey as ECPublicKey?, privateKey as ECPrivateKey?)
        return algorithm
    }
