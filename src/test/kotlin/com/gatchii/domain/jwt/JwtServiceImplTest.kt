...
class JwtServiceImplTest {
  ...
  fun `test generateJwt with valid claim`() = runTest {
    ...
    // Then
    assertTrue(
      converted?.getClaim("username")!!.asString() == "testUser"
    )
    assertTrue(
      converted.getClaim("role")!!.asString() == "user"
    )
  }
  ...
  fun `test generateJwt with empty claim`() = runTest {
    ...
    // Then
    val username = converted?.getClaim("username")?.asString()
    val role = converted?.getClaim("role")?.asString()

    assertFalse(username != null) // username 없어야 함
    assertFalse(role != null) // role 없어야 함
  }
